...
ggplot(r_dis_type_fmt, aes(dis_type, share_est, fill = dis_type)) +
  ...
  geom_errorbar(aes(ymax = share_est + share_moe, ymin = share_est - share_moe),
...
dis_wrkclass_overview04 <- dis_wrkclass_overview03 %>% 
  ...
  group_by(DATA_YEAR, COUNTY, occp_abbr, DIS) %>% 
  summarise(count, count_moe, share = count/count_denom, share_moe = moe_prop(count, count_denom, count_moe, moe_denom)) %>% 
  left_join(occ_lu, by = "occp_abbr")

dis_wrkclass_overview_fmt <- dis_wrkclass_overview04 %>% 
  select(DATA_YEAR, COUNTY, occp_abbr, definition, DIS, count, count_moe, share, share_moe)
