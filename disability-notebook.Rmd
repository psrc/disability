...
dis_modsect_pums <- get_psrc_pums(span = 1,                                     # Denoting ACS 5-year estimates; 1-year also available
                                  dyear = 2021,                                 # Last data year of span
                                  level = "p",                                  # Unit of analysis == household ("p" used for person)
...
ggplot(dis_modsect_overview_fmt_plot, aes(x = reorder(sector, -share), y = share)) +
  geom_col(position = 'dodge') +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +
