...
- 2022 ACS 5-year B18120 Employment Status by Disability Status and Type (Civilian Non-institutionalized Population 18 to 64 Years)
<!-- B18121 Work Experience by Disability Status and Type -->
<!-- B18130 Age by Disability Status by Poverty Status -->

- 2020 5-year ACS PUMS variables or 2021 1-year ACS PUMS variables 
  - DIS (disability flag)
...
outdir <- "T:\\2023November\\Christy\\disability_emp"
```
...
dis_covemp_fmt <- bind_rows(dis_covemp_2021, dis_covemp_2020)
```

```{r export to xlsx, eval=FALSE}

write.xlsx(dis_covemp_fmt, file.path(outdir, "disability_covemp_pums_2020_2021.xlsx"))
```
...
ggplot(dis_covemp_fmt_plot, aes(x = reorder(sector, -share), y = share, fill = DATA_YEAR)) +
  ...
  labs(y = NULL,
       ...
       caption = 'Source: 2020 5-year ACS PUMS, 2021 1-year ACS PUMS.'
       ) +
  theme(legend.title = element_blank(),
        axis.ticks.x = element_blank(),
        axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=.5))
...
- In both 2020 and 2021, about 2% of the region's population report a disability while working in the Services sector.
- About .5% of the region's population report a disability and work in the Retail or Manufacturing sector.