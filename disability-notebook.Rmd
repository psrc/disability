...
outdir <- "T:\\2024October\\Christy\\disability_emp"
acs_year <- 2023
```
...
```{r Work experience by disability status and duration chart}
...
```{r PUMS Covered Employment job sectors, include=FALSE}
...
```{r PUMS Covered Employment job sectors chart, include=TRUE}
...
ggplot(dis_covemp_fmt_plot, aes(x = reorder(sector, -share), y = share, fill = DATA_YEAR)) +
  ...
  labs(y = NULL,
       ...
       caption = 'Source: 2020 5-year ACS PUMS, 2021, 2022, 2023 1-year ACS PUMS'
  ) +
  theme(legend.title = element_blank(),
...
# Regional Median Earnings by Disability Status (PUMS)

```{r Median Earnings by Disability Status, eval=TRUE, include=TRUE}
...
m_earn_pums_fil <- m_earn_pums %>% 
  filter(ESR %in% c(1,2,4,5) & !is.na(ESR) & between(AGEP, 18,64))
  # filter((grepl("^(Civilian|Armed) ", as.character(ESR)) & !is.na(ESR) & between(AGEP, 18, 64))) # for prior to 2023 data
...
```{r Median Earnings by Disability Status table, include=TRUE}
datatable(m_earn) %>%
...
```{r Labor Force by Disability Status Data}
# 1. Employment graph
...
```{r Labor Force by Disability Status Plot}
...
df_lf_dis_unemp_p <- static_column_chart(
  ...
  ylabel = NULL, 
  est = "percent",
  title = paste("Percent Unemployed by Disability Status", acs_year),
  subtitle = "",
...
# # jpeg(file=here::here(paste0("images/B18120_in_labor_force_", acs_year,".jpeg")), height = 532, width = 800)
# jpeg(file=file.path(outdir, paste0("B18120_in_labor_force_", acs_year,".jpeg")), height = 532, width = 800)
# print(df_lf_dis_unemp_p)
# dev.off()

```

```{r  2023 CovEmp Sectors, include=TRUE}

d <- get(paste0("dis_covemp_", acs_year))
dis_type <- unique(d[['DIS']])
...
# write.xlsx(dcemp_23_2, "cov_emp_23_chart.xlsx")
# df <- read.xlsx("cov_emp_23_chart.xlsx")
...
df_bar <- static_bar_chart(
  ...
  ylabel = NULL,
  est = "percent",
  title = "Disability Status by Covered Employment Job Sectors in 2023",
  subtitle = NULL,
  source = "ACS 1-year 2023 PUMS",
  alt = NULL,
) +
  theme(plot.margin = margin(t = 1, r = 1, b = 1, l = 1, "cm"))
...
# jpeg(file=here::here("images/dis_status_by_covemp_23_2.jpeg"), height = 532, width = 800)
# print(df_bar)
# dev.off()

# copy and paste to outdir
...
pov_df_aa_p <- static_column_chart(
  ...
  ylabel = NULL, 
  est = "percent",
  title = paste("Disability Status Below Poverty Level in", acs_year ),
  subtitle = "Income in the past 12 months",
  source = paste("ACS 1-year", acs_year, "C18130 Poverty Status by Disability Status and Age"),
  alt = NULL,
) +
  theme(plot.margin = margin(t = 1, r = 1, b = 1, l = 1, "cm"))

pov_df_aa_p
# # jpeg(file=here::here(paste0("images/dis_status_by_pov_lev_", acs_year ,".jpeg")), height = 532, width = 800)
# jpeg(file= file.path(outdir, paste0("dis_status_by_pov_lev_", acs_year ,".jpeg")), height = 532, width = 800)
# print(pov_df_aa_p)
# dev.off()
```

```{r notebook, eval=FALSE}
this_nb_html <- list.files(here::here(), 
                           pattern = "disability-notebook.nb.html",
                           full.names = TRUE)
file.copy(from = this_nb_html, 
          to = file.path(outdir, "disability-notebook-2023.nb.html"), 
          overwrite = TRUE)
```
